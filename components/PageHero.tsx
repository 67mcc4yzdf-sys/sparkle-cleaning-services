...
export default function PageHero({ eyebrow, title, description, showCta = true }: { eyebrow: string; title: string; description: string; showCta?: boolean }) {
  return <section className="border-b border-pink-100 bg-[#fff3f7] py-16 sm:py-20">
    <div className="shell text-left">
      ...
      {showCta && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: .15 }}><Link href="/contact#quote" className="button-primary mt-8">{business.ctaPrimary} <ArrowRight size={17} /></Link></motion.div>}
    </div>
  </section>;
}
