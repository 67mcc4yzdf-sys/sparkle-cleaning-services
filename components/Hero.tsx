...
import { Check, Heart, Phone, Sparkles, Star } from "lucide-react";
    ...
    <section className="relative overflow-hidden border-b border-pink-100 bg-[#fff3f7] lg:min-h-[720px]">
      ...
      <div className="shell relative z-10 grid gap-10 py-12 sm:py-16 lg:min-h-[720px] lg:grid-cols-[1fr_.92fr] lg:items-center lg:py-20">
        <div className="max-w-2xl">
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mb-7 inline-flex items-center gap-3 rounded-full border border-pink-200 bg-white/75 px-4 py-2 shadow-sm">
            <span className="relative grid size-10 place-items-center rounded-full border border-pink-200 bg-pink-50 text-base font-black text-[var(--primary)]">
              S
              <Sparkles className="absolute -right-1 -top-1 text-pink-400" size={12} />
            </span>
            <div>
              <p className="flex items-center gap-1 font-[var(--font-accent)] text-2xl leading-none text-[var(--primary)]">{business.logoText}<Heart className="mt-1 text-pink-400" size={13} fill="currentColor" /></p>
              <p className="mt-1 text-[10px] font-extrabold uppercase tracking-[.2em] text-pink-500">Cleaning Services</p>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .28 }} className="mt-8 flex flex-col gap-3 sm:flex-row">
            ...
            <a href={`tel:${business.phoneHref}`} className="button-secondary"><Phone size={17} />Call {business.phone}</a>
          </motion.div>
        ...
        <motion.div initial={{ opacity: 0, scale: .98 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: .18, duration: .6 }} className="relative">
          <div className="absolute -left-4 -top-4 z-10 rounded-full bg-white px-4 py-2 text-sm font-extrabold text-[var(--primary)] shadow-lg shadow-pink-200/50"><Sparkles className="mr-1 inline" size={15} />Premium local clean</div>
          ...
          <div className="absolute -bottom-5 right-6 rounded-3xl border border-pink-100 bg-white/95 px-5 py-4 shadow-xl">
            <p className="font-[var(--font-accent)] text-2xl leading-none text-[var(--primary)]">just sit back and relax</p>
          </div>
        </motion.div>
      </div>
    </section>
